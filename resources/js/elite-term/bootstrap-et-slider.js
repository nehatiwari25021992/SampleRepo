...
var r = $('#R2').slider();
	...
	var x = slideEvt.value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	$("#et-slider-range").text(x);
	});

	$( "#R2" ).change(function(val) {
  	var z = val.value.newValue;
  	var txt = z.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	$("#et-slider-range").text(txt);
});
